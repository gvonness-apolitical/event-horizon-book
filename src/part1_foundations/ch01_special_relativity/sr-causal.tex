...
% FIGURE PLACEHOLDER
% Description: A 2+1 spacetime diagram (t vs x,y) showing the light cone at
%   an event p, with the future cone (upward) and past cone (downward).
%   A timelike worldline passes through the interior of the cone. A spacelike
%   surface lies outside. The chronological future I^+(p) and causal future
%   J^+(p) regions are shaded, with J^+ boundary = cone surface.
% Source: tikz diagram or matplotlib 3D projection
% Parameters: t vertical, x and y horizontal. Future cone above origin,
%   past cone below. Shade I^+(p) in translucent gold, J^+(p) boundary
%   (cone surface) in crimson. Show a timelike curve (gold) inside the cone
%   and a spacelike separation (blue) outside. Label I^+(p), J^+(p), and
%   the "elsewhere" (spacelike) region. Use EH colour palette.
% Caption draft: The light cone at an event $p$ in Minkowski spacetime.
%   The chronological future $I^+(p)$ (gold interior) contains all events
...
metric.
\histnote{Penrose's conformal diagrams \cite{TODO:Penrose1963} make the global
