...
A \emph{four-vector} $V$ can be expressed in two equivalent ways.  The
\emph{contravariant} components $V^\mu$ carry upper indices; the
\emph{covariant} components $V_\mu$ carry lower indices.  The metric
translates between them.  Lowering an index:
%
\begin{equation}
  V_\mu = \eta_{\mu\nu}\, V^\nu \,.
...
Minkowski spacetime, the inverse has the same components as the metric itself:
%
\begin{equation}
  \eta^{\mu\nu}
    = \operatorname{diag}(-1,\, +1,\, +1,\, +1) \,.
...
Think of the metric as a translation dictionary between two dialects of the
same language.  Contravariant and covariant components are two descriptions of
the same physical vector: one suited to velocities ($\d x^\mu / \d\tau$), the
other to gradients ($\partial_\mu \phi$).  The metric converts freely between
them.  This analogy carries through to curved spacetime, where the dictionary
changes from point to point but the concept of two representations persists.
...
The metric defines an inner product between any two four-vectors $A^\mu$ and
$B^\mu$:
%
\begin{equation}
  A \cdot B
...
% FIGURE PLACEHOLDER
% Description: A 1+1 spacetime diagram (t vs x) showing the light cone
%   (two 45-degree lines from the origin), with a timelike vector inside
%   the future cone, a spacelike vector outside, and a null vector along
%   the cone edge. Shaded future/past regions.
% Source: tikz diagram
% Parameters: Standard axes, t vertical, x horizontal. Light cone at 45 deg.
%   Gold vector (timelike, V^2 < 0) inside future cone. Blue vector
%   (spacelike, V^2 > 0) roughly horizontal. Crimson vector (null, V^2 = 0)
%   along cone edge. Use EH colour palette.
% Caption draft: Spacetime diagram of 1+1 Minkowski space, showing the
%   three causal classes of four-vectors. Timelike vectors (gold) lie inside
%   the light cone and have negative norm $\eta_{\mu\nu} V^\mu V^\nu < 0$;
%   spacelike vectors (blue) lie outside and have positive norm; null
%   vectors (crimson) lie exactly on the cone with vanishing norm. The
%   photon trajectories computed by the ray tracer are always null---they
%   live on this cone and its curved-spacetime generalisation.
% Label: fig:minkowski-lightcone

\paragraph{A worked example.}

Consider a photon emitted at the surface of a $10\,M_\odot$ black hole (at the
Schwarzschild radius $r_s = 2GM/c^2$; in geometrised units, $r_s = 2M
\approx \SI{29.5}{\kilo\metre}$) and detected by a distant observer.  In flat
spacetime---which is a reasonable approximation far from the hole---the
photon's displacement four-vector over a coordinate time $\Delta t$ is
%
\begin{equation}
...
\end{equation}
%
for a photon travelling in the $x$-direction (recall $c = 1$).
The interval is
%
\begin{equation}
  \Delta s^2
...
Now consider a massive particle---say, a test mass in a circular orbit at $r =
6M \approx \SI{88.6}{\kilo\metre}$ (the innermost stable circular orbit in
Schwarzschild).  Over a small proper-time interval $\Delta\tau$, the
displacement satisfies $\Delta s^2 = -\Delta\tau^2 < 0$: timelike, as it must
be for a massive particle.
\xrefnote{Circular orbits and the ISCO are treated properly in
\cref{sec:geo-timelike}.}
