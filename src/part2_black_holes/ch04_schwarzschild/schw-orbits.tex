...
where $V'_{\text{eff}} = 0$ simultaneously).  For timelike geodesics
($\kappa = -1$),
%
\begin{equation}
...
% FIGURE PLACEHOLDER
% Description: Effective potential V_eff(r) for the Schwarzschild
%   spacetime.  Two panels: (a) null geodesics showing V_eff/L^2 vs
%   r/M with the maximum at r=3M (photon sphere) marked, and the
%   critical impact parameter b_c = 3sqrt(3)M indicated; (b) timelike
%   geodesics showing V_eff vs r/M for several values of L/M,
%   illustrating the stable minimum and unstable maximum that merge
%   at the ISCO (L^2 = 12M^2, r = 6M).  Vertical dashed lines at
%   r = 2M (horizon), r = 3M (photon sphere), r = 6M (ISCO).
% Source: data plot
% Parameters: M = 1, L/M = 3.0, 3.464 (ISCO), 4.0, 5.0 for timelike;
%   single curve for null.  EH colour palette.
% Caption draft: Effective potential for geodesics in the Schwarzschild
%   spacetime with $M = 1$.  (a) Null geodesics:
%   $V_{\text{eff}}/L^2$ has a single maximum at the photon sphere
%   $r = 3M$; photons with impact parameter $b < b_c = 3\sqrt{3}\,M$
%   are captured.  (b) Timelike geodesics: $V_{\text{eff}}$ for
%   several angular momenta $L$.  At $L^2 = 12M^2$ (bold curve), the
%   local minimum and maximum merge at the ISCO $r = 6M$; below this
%   angular momentum, no stable circular orbit exists.  The vertical
%   dashed line marks the event horizon $r = 2M$.
% Short caption: Effective potential for Schwarzschild geodesics
